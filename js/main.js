...
document.addEventListener('DOMContentLoaded', () => {
 ...
 features: [] // Will be populated from data
 },
 trails: {
 title: '🛤️ Cart Trails',
 ...
 sources: 'Manitoba Archives, Hudson's Bay Company records, Historical maps and journals',
 features: [] // Will be populated from data
 },
 locations: {
 ...
 color: '#FF9800',
 count: 2,
 description: 'By 1870, commercial hunting and westward expansion had already reduced the range. The buffalo were still present but their territory was shrinking rapidly. This period coincides with Manitoba's entry into Confederation.',
 polygons: []
 },
 {
 name: '1889 Range',
 ...
 // Layer info buttons
 const layerInfoBtns = document.querySelectorAll('.layer-info-btn');
 layerInfoBtns.forEach(btn => {
 ...
 <label for="layerToggleCheckbox">Enable ${info.title.split(' ')[1] || 'Layer'}</label>
 </div>
 `;
 ...
 layerInfo.waterways.features = geojsonData.features.slice(0, 10).map(f => ({
 name: f.properties?.name || 'Waterway',
 description: f.properties?.description || 'A water feature in the homeland'
 }));
 }
 ...
 // Update layer info
 if (geojsonData.features && geojsonData.features.length > 0) {
 layerInfo.trails.features = geojsonData.features.slice(0, 10).map(f => ({
 name: f.properties?.name || 'Trail',
 description: f.properties?.description || 'A historic cart trail'
 }));
 }
 ...
 // Update layer info
 if (geojsonData.features && geojsonData.features.length > 0) {
 layerInfo.locations.features = geojsonData.features.slice(0, 10).map(f => ({
 name: f.properties?.name || 'Location',
 description: f.properties?.description || 'A Métis community or settlement'
 }));
 }
 ...
 // Update layer info with polygon data
 if (geojsonData.features && geojsonData.features.length > 0) {
 ...
 const desc = f.properties?.description || `Buffalo range polygon ${i + 1}`;
 
 if (name.includes('Original')) {
 originalPolygons.push({ name, description: desc });
 } else if (name.includes('1870')) {
 polygons1870.push({ name, description: desc });
 } else if (name.includes('1889')) {
 polygons1889.push({ name, description: desc });
 }
 });
 
 layerInfo.buffalo.eras[0].polygons = originalPolygons;
 layerInfo.buffalo.eras[1].polygons = polygons1870;
 layerInfo.buffalo.eras[2].polygons = polygons1889;
 
 // Update counts
 layerInfo.buffalo.eras[0].count = originalPolygons.length || 1;
 layerInfo.buffalo.eras[1].count = polygons1870.length || 2;
 layerInfo.buffalo.eras[2].count = polygons1889.length || 6;
 }
